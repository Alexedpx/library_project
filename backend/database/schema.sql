...
CREATE TABLE book (
    id INT PRIMARY KEY AUTO_INCREMENT NOT NULL, image VARCHAR(255) NULL, titre VARCHAR(80) NOT NULL, auteur VARCHAR(80) NOT NULL, nombre_pages INT NOT NULL, date DATE NOT NULL, langue VARCHAR(80) NOT NULL, categorie VARCHAR(80) NOT NULL, description VARCHAR(750) NULL, commentaire VARCHAR(255) NULL, statut ENUM('Non lu', 'En cours', 'Lu') NOT NULL DEFAULT 'Non lu', user_id INT, CONSTRAINT fk_book_user FOREIGN KEY (user_id) REFERENCES user (id) ON DELETE CASCADE ON UPDATE NO ACTION
);
